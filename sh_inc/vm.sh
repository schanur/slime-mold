...
# Check if OS has hardware virtualization support. If OS supports KVM,
...
# Generate a list of IPs used by the QEMU slirp network backend.
...
# Generate a list of VLAN ids used by the QEMU slirp network backend.
...
# Create an overlay image. In an overlay image, only the difference to
# the base image is saved. This feature saves a lot of memory if you
# need multible instances of the same OS installation.
...
# Print the status of a virtual machine.  The status contains the
# information if the VM is running and the corresponding log file,
# which is generated by the async_qemu.sh script.
function vm__log
{
    local VM_NAME="${1}"
    local VM_RUNNING
...
# Print the VM name to a corresponding image file.
